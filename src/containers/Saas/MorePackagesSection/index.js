...
const MorePackagesSection = ({
  ...
  const [morePackState, setMorePackState] = useState(false)

  const [morePackTextActive, setMorePackTextActive] = useState(true);

  return (
    <>
      <Box {...viewMoreWrapper}>
        <Box {...secTitleWrapper} style={{ background: "white" }} >
          <Box {...viewMoreButtonWrapper}>
            <Button {...button}
              onClick={() => {
                if (morePackTextActive) {
                  setMorePackState(true);
                  setMorePackTextActive(false);
                }
                else {
                  setMorePackState(false);
                  setMorePackTextActive(true);
                }
              }}
              title={morePackTextActive ? "View More Packages + " : "Hide More Packages - "} />
          </Box>
        </Box>
      </Box>

      <Box  {...sectionWrapper} id="more_packages_section" style={{ display: morePackState ? 'block' : 'none' }}>
        <Container>
          <Box {...secTitleWrapper}>
            {/* <Text {...secText} /> */}
            <Heading {...secHeading} />
          ...
          <Box {...row} >
            <GlideCarousel
              carouselSelector="more-pricing"
              options={morePackagesCarouselOptions}
              controls={true}
              bullets={true}
              numberOfBullets={3}
            >
...
MorePackagesSection.propTypes = {
  sectionWrapper: PropTypes.object,
  viewMoreWrapper: PropTypes.object,
  viewMoreButtonWrapper: PropTypes.object,
...
MorePackagesSection.defaultProps = {
  viewMoreWrapper: {
    as: 'section',
    pt: ['20px', '20px', '20px', '30px', '30px'],
...
export default MorePackagesSection; 
